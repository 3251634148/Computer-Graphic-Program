...
// TODO:补充不同方块的信息
		...
		{{glm::vec2(0, 0), glm::vec2(-1,0), glm::vec2(1, -1), glm::vec2(0, -1)},	//   "Z"
		{glm::vec2(1, 1), glm::vec2(0, 0), glm::vec2(0,-1), glm::vec2(1, 0)},
		{glm::vec2(0, 0), glm::vec2(-1,0), glm::vec2(1, -1), glm::vec2(0, -1)},
		{glm::vec2(1, 1), glm::vec2(0, 0), glm::vec2(0,-1), glm::vec2(1, 0)}},

		{{glm::vec2(0, 0), glm::vec2(1,0), glm::vec2(-1, -1), glm::vec2(0, -1)},	//   "S"
		...
		{{glm::vec2(-1, 0), glm::vec2(0,0), glm::vec2(-1, -1), glm::vec2(0, -1)},	//   "O"
		{glm::vec2(-1, 0), glm::vec2(0,0), glm::vec2(-1, -1), glm::vec2(0, -1)},
		{glm::vec2(-1, 0), glm::vec2(0,0), glm::vec2(-1, -1), glm::vec2(0, -1)},
		{glm::vec2(-1, 0), glm::vec2(0,0), glm::vec2(-1, -1), glm::vec2(0, -1)}}
};
int shapeType;
...
glm::vec4 tempColor = glm::vec4(0.0, 0.0, 0.0, 1.0);
...
//记录格子颜色，辅助行清除
glm::vec4 pos_colors[board_width][board_height];
...
void changecellcolour(glm::vec2 pos, glm::vec4 colour)
{
	...
	//记录每个格子的颜色
	pos_colors[(int)pos.x][(int)pos.y] = colour;
}
...
void newtile()
{
	...
	// @TODO:随机生成方块
	shapeType = (rand() % 7);
	int colorType = (rand() % 5) + 1;
	if (shapeType == 0) {
		for (int i = 0; i < 4; i++)
		{
			tile[i] = allRotationsshape[shapeType][0][i];
		}
	}
	else if (shapeType == 1) {
		for (int i = 0; i < 4; i++)
...
// @TODO：检查在cellpos位置的格子是否被填充或者是否在棋盘格的边界范围内,以及是否与其他方块冲突(即那个位置是否被填充)
bool checkvalid(glm::vec2 cellpos)
{
	int x = cellpos.x, y = cellpos.y;
	if((cellpos.x >=0) && (cellpos.x < board_width) && (cellpos.y >= 0) && (cellpos.y < board_height) && !board[x][y])
		return true;
	else
		return false;
}
...
void rotate()
{
	...
	// 检查当前旋转之后的位置的有效性
	if (checkvalid((allRotationsshape[shapeType][nextrotation][0]) + tilepos)
		&& checkvalid((allRotationsshape[shapeType][nextrotation][1]) + tilepos)
		&& checkvalid((allRotationsshape[shapeType][nextrotation][2]) + tilepos)
		&& checkvalid((allRotationsshape[shapeType][nextrotation][3]) + tilepos))
	{
		// 更新旋转，将当前方块设置为旋转之后的方块
		rotation = nextrotation;
		for (int i = 0; i < 4; i++)
			tile[i] = allRotationsshape[shapeType][rotation][i];

		updatetile();
	}
}

// TODO：检查棋盘格在row行有没有被填充满
void checkfullrow(int row)
{
	...
	// 如果已经满了，则消去该行，并将在它上面的所有格子下移一格
	for (int j = row; j < board_height; j++) {
		for (int i = 0; i < board_width; i++) {
			if (j + 1 < board_height && board[i][j + 1] == true) {
				// 如果该行上面一行有格子，就复制到当前行
				changecellcolour(glm::vec2(i, j), pos_colors[i][j + 1]);
				board[i][j] = true;
			}
			else {
...
void settile()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		board[x][y] = true;
		// 并将相应位置的颜色修改
		changecellcolour(glm::vec2(x, y), tempColor);
	}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	if(!gameover)
	{
		...
		{	
			...
			case GLFW_KEY_SPACE: // 空格键，可以一下子降落到底部
				if (action == GLFW_PRESS) {
...
int main(int argc, char **argv)
{
	...
	// 创建窗口。
	GLFWwindow* window = glfwCreateWindow(500, 900, "2022150153-lcx-Mid-Term-Skeleton-Code", NULL, NULL);
	if (window == NULL)
	{
