...
void TriMesh::computeTriangleNormals()
{
	...
	for (size_t i = 0; i < faces.size(); i++) {
		...
		glm::vec3 norm;		
		glm::vec3 p1 = vertex_positions[face.x], p2 = vertex_positions[face.y], p3 = vertex_positions[face.z];
		glm::vec3 v1 = glm::vec3(p1 - p2), v2 = glm::vec3(p3 - p2);
		norm = glm::cross(v1, v2);
		face_normals[i] = normalize(norm);
	}
}
